...
test("link audit representative project roots are derived from the project registry", async () => {
  ...
  assert.ok(routes.includes("/heat-defense/pages/treatment-system-consensus"));
  assert.ok(routes.includes("/heat-defense/pages/core-temperature-cooling"));
  assert.ok(routes.includes("/heat-defense/pages/heat-tolerance"));
  assert.ok(routes.includes("/heat-defense/pages/challenge"));
...
test("integrated project runtime ownership guards Next route handoff", async () => {
  ...
  assert.equal(
    ...
    "next-home-about-rule-guide-consensus-cooling-tolerance-and-challenge-owned-static-deep-pages-pending",
  );
  assert.equal(
