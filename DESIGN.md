...
生成式插画只用于症状识别、场景判断等需要快速建立视觉印象的内容，不用于替代医学定义、流程动作或风险说明。

- 统一画幅：横向 `3:2`，项目资产优先导出为 `1200×800` WebP。
- 统一风格：新构成主义战地医疗海报、平面剪纸和丝网印刷质感，克制的旧纸纹理，清晰人物姿态。
- 固定色板：米白纸底 `#F4ECDC`、战地深蓝 `#12313C`、主信号红 `#D93025`，临床浅蓝 `#78C7E7` 只做少量辅助。
- 图片内禁止文字、字母、数字、Logo、水印、界面、医学标签、血腥画面和夸张表情。
- 专业文字必须放在图片外的独立高对比区域；每张图必须提供描述人物状态的中文 `alt`。
- 同组图片使用同一提示词骨架，只替换人物状态和构图动作；不得让生成图新增原文没有的诊断结论。
