...
-- 创建级联删除药品的函数
CREATE OR REPLACE FUNCTION public.cascade_delete_medicine(medicine_id_param UUID)
RETURNS BOOLEAN AS $
DECLARE
...
-- 创建级联删除批次的函数
CREATE OR REPLACE FUNCTION public.cascade_delete_batch(batch_id_param UUID)
RETURNS BOOLEAN AS $
DECLARE
  transaction_count INTEGER;
BEGIN
...
-- 创建安全删除药品的函数（带确认）
CREATE OR REPLACE FUNCTION public.safe_delete_medicine(
...
-- 创建查询药品关联数据的函数
CREATE OR REPLACE FUNCTION public.get_medicine_dependencies(medicine_id_param UUID)
RETURNS JSON AS $
DECLARE
  medicine_info RECORD;
  transaction_count INTEGER;
...
-- 创建查询批次关联数据的函数
CREATE OR REPLACE FUNCTION public.get_batch_dependencies(batch_id_param UUID)
RETURNS JSON AS $
DECLARE
  batch_info RECORD;
  transaction_count INTEGER;
