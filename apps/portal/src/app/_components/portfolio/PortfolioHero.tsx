...
export function PortfolioHero() {
  return (
    <header
      className={`${styles.posterSurface} relative px-6 md:px-8 pt-20 pb-14 border-b-4 border-constructivism-red dark:border-constructivism-red overflow-hidden transition-colors duration-300`}
    >
      <div
      ...
      />
      <div
        className={`${styles.ecgLine} absolute left-0 right-0 top-[46%]`}
        aria-hidden="true"
      />

      <div
        className={`${styles.heroContent} relative z-10 max-w-7xl mx-auto w-full`}
      >
        <div>
          <a
            href="/"
          ...
          >
            <span
              className="h-4 w-4 bg-constructivism-red"
              aria-hidden="true"
            />
            红医师
          </a>
          ...
          <h1 className="max-w-5xl text-7xl md:text-9xl lg:text-[10rem] font-black leading-[0.86] text-constructivism-red transition-colors duration-300">
            红医师
          </h1>
            ...
            {homepageTaskEntries.map((entry) => {
              ...
              return (
                <a
                  key={entry.id}
                  href={entry.href}
                  className={`${styles.taskCard} group border-3 border-black bg-[#f4ecdc] p-4 text-left dark:border-white/30 dark:bg-neutral-950`}
                >
                  ...
                  <span className="mt-4 block font-mono text-[0.68rem] leading-5 text-neutral-500 dark:text-neutral-500">
                    {project?.shortTitle ?? entry.projectId} ·{" "}
                    {entry.sourceNote}
                  </span>
                </a>
              );
