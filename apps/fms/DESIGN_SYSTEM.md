...
## 概述
FMS功能性动作筛查系统采用**红医师品牌系统**设计风格，灵感来源于OpenAI的设计理念，强调简洁、功能性和优雅的用户体验。
...
/* 标题系统 */
.hys-title: clamp(2.5rem, 5vw, 3.5rem)  /* 主标题 */
.hys-subtitle: clamp(1.25rem, 2.5vw, 1.375rem)  /* 副标题 */
.hys-text: 1.125rem  /* 正文 */

/* 移动端适配 */
@media (max-width: 768px) {
  .hys-title: 2rem
  .hys-subtitle: 1.125rem
  .hys-text: 1rem
}
```
...
```css
/* 页面级间距 */
.hys-section: clamp(2rem, 6vw, 8rem) 0  /* 上下间距 */
.hys-container: 0 clamp(1rem, 3vw, 2rem)  /* 左右内边距 */

/* 组件间距 */
.hys-grid: clamp(1.5rem, 3vw, 3rem)  /* 网格间距 */

/* 移动端优化 */
@media (max-width: 768px) {
  .hys-section: 2rem 0
  .hys-container: 0 1rem
  .hys-grid: 1.5rem
}
```
...
<div className="hys-section">
  <div className="hys-container max-w-6xl">
    ...
    <div className="text-center mb-16 md:mb-20 minimal-fade-in">
      <h1 className="hys-title">页面标题</h1>
      <p className="hys-subtitle">页面描述</p>
    </div>
